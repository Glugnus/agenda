...
export default function FormWithFormik({
  ...
  const validationSchema = Yup.object().shape({
    ...
    location: Yup.string()
      .when("isOnline", {
        is: true,
        then: (schema) => schema.url("L'Url est invalide"),
        otherwise: (schema) => schema.notRequired(),
      })
      .notRequired(),
    phoneNumber: Yup.string().matches(/^[0]\d{9}$/, {
      message: "Le numéro de téléphone est invalide",
    }),
    startDate: Yup.date(),
    endDate: Yup.date().min(
      Yup.ref("startDate", {
        message: "La fin de l'évènement doit être après le début",
      }),
    ),
